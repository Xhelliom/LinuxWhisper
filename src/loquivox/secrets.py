...
Keys live in ``~/.config/loquivox/secrets.env`` (``KEY=value`` lines,
...
SECRETS_FILE: Path = Path.home() / ".config" / "loquivox" / "secrets.env"
...
def save_secrets(values: Dict[str, str]) -> None:
    ...
    lines = ["# Loquivox API keys — managed by the settings UI. chmod 600.\n"]
