...
Usage:
    from loquivox.platform import get_clipboard, get_input, get_screenshot
...
def get_clipboard() -> ClipboardBackend:
    ...
    if _clipboard is None:
        if SESSION_TYPE == "wayland":
            from loquivox.platform.wayland import WaylandClipboard
            _clipboard = WaylandClipboard()
        else:
            from loquivox.platform.x11 import X11Clipboard
...
def get_input() -> InputBackend:
    ...
    if _input is None:
        if SESSION_TYPE == "wayland":
            from loquivox.platform.wayland import WaylandInput
            _input = WaylandInput()
        else:
            from loquivox.platform.x11 import X11Input
            _input = X11Input()
    return _input
...
def get_screenshot() -> ScreenshotBackend:
    ...
    if _screenshot is None:
        if SESSION_TYPE == "wayland":
            from loquivox.platform.wayland import WaylandScreenshot
            _screenshot = WaylandScreenshot()
        else:
            from loquivox.platform.x11 import X11Screenshot
